...
vector<string> Vehicle::successor_states() {
  if(this->lane == 0){
    return {"KL","LCR","LCL","PLCL","PLCR"};
  }
  else if (this->lane < lanes_available-1){
    return {"KL","LCR","LCL","PLCL","PLCR"};
  }
  return {"KL","LCR","LCL","PLCL","PLCR"};
}
...
void Vehicle::update_state(map<int,vector < vector<int> > > predictions) {
  ...
  string best_state = "";
  for(string & state: successor_states()) {
    cout << "current lane " << lane;
    cout << "trying: " << state << endl;
    double cost = 0;
    int horizon = predictions.size();// time to keep the states
  ...
  /*
    ...
    */
  cout << "best was " << best_state;
    state = best_state; // this is an example of how you change state.
